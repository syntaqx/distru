...
import { listPaymentMethods, upsertPaymentMethod, paymentMethodToApi } from "@/lib/modules/sales";
...
export async function GET(req: Request) {
  ...
  const { items, total } = await listPaymentMethods(auth.ctx, { limit: PAGE_SIZE, offset });
  return listEnvelope(req, items.map(paymentMethodToApi), offset, total);
}
...
export async function POST(req: Request) {
  ...
  try {
    const { row, created } = await upsertPaymentMethod(auth.ctx, {
      id: body.id as string | undefined,
      name: body.name as string | undefined,
    });
    return Response.json({ data: paymentMethodToApi(row) }, { status: created ? 201 : 200 });
  } catch (err) {
