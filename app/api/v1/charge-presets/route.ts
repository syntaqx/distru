...
import { listChargePresets, upsertChargePreset, chargePresetToApi } from "@/lib/modules/sales";
...
export async function GET(req: Request) {
  ...
  const { items, total } = await listChargePresets(auth.ctx, { limit: PAGE_SIZE, offset });
  return listEnvelope(req, items.map(chargePresetToApi), offset, total);
}
...
export async function POST(req: Request) {
  ...
  try {
    const { row, created } = await upsertChargePreset(auth.ctx, {
      id: body.id as string | undefined,
      name: body.name as string | undefined,
      kind: body.kind as string | undefined,
      amount: body.amount as number | undefined,
    });
    return Response.json({ data: chargePresetToApi(row) }, { status: created ? 201 : 200 });
  } catch (err) {
