...
flowchart TD
    subgraph Faces
      ...
      WF[Workflow graph engine - triggers/agent/action/if nodes + cron tick]
    end
...
### 3.8 Workflow graph model (`lib/harness/graph/`)

Automations are a directed graph, deliberately **n8n-shaped**: a workflow is `{ nodes, connections }` where `connections` is keyed by source node and, per kind, an array of output ports → endpoints - the same document n8n uses, so the mental model transfers and an import/export shim stays possible. Two deliberate, documented deviations: connections key on stable node **id** (not name), and our tools surface as **`tool` sub-nodes** attached to an agent via an `ai_tool` connection (the n8n AI-Agent sub-node pattern). The one thing that's *ours*, not n8n's: an **agent** node is a first-class citizen - it runs the same harness turn as chat (§3.3), scoped to exactly the tools wired into its port, auto-approving because unattended. Deterministic `action` (one tool, no LLM), `if` (two-output branch), and `set` (context transform) nodes compose in the same graph, exactly like n8n mixes an "If" node with an "AI Agent."

The **executor** (`executor.ts`) walks `main` connections from the trigger in flow order, running each node once against a shared run-context that later nodes read via `{{ nodes.<id>.summary }}` / `{{ trigger.<field> }}` templates; agent nodes get their own conversation (its transcript is the node's audit trail), and per-node results are recorded on the run. A tiny standard-cron evaluator (`schedule.ts`) arms `nextRunAt`, and a `tick` endpoint fires due workflows and re-arms them. The graph is edited on a **React Flow** canvas; pure converters (`reactflow.ts`) map the canonical graph ↔ React Flow's `{ nodes, edges }` (roundtrip-tested), and `authoring.ts` turns a plain-language prompt into a validated graph in one structured model call. A **Visual / JSON toggle** exposes the canonical `{ nodes, connections }` document directly - copy it to share an automation, paste one in to import it. This is the "trigger-agnostic by construction" claim (§3.5) made visual: the executor has no idea whether a human clicked Run or a cron fired the tick.

**Real-world outcomes (produce → deliver → notify).** A workflow doesn't just print text: an agent node calls `save_report` to persist its output as a durable **artifact** that lives in a **Reports** section (rendered, downloadable). Delivery is the outbound side of the integration seam (§5): `email_report` and `upload_to_drive` push an artifact to an inbox or a Drive folder through `EmailProvider` / `DriveProvider` (mock-connected by default, `none` = "connect it in Settings → Integrations"). And every run drops an in-app **notification** (a `notifications` row) with a deep link to its results, surfaced both on the topbar bell and on a full **Notifications** inbox page (GitHub-style, in the nav under Dashboard). So the seeded low-stock automation becomes: *schedule → agent (build + `save_report`) → the report appears in Reports, is emailed/uploaded, and the bell links you straight to it.*

This also ties into **Insights**. The 18 standard analytical reports now live in one **report registry** (`lib/modules/reports/insights-reports.ts`) - a single definition (columns + `run(ctx)`) per report that is the sole source of truth for the public `/reports/*` API (each route collapses to a 4-line factory call), the Insights UI list, *and* a `generate_report` tool. So an operator can hit **"Save to Reports"** on any Insights report to snapshot it as an artifact, and an automation can `generate_report("sales-by-product")` on a schedule and email the result - the numbers are guaranteed identical everywhere because there's one implementation.
...
- **Automations:** a visual, n8n-shaped **workflow engine** where an **AI agent is a first-class node**. A workflow is a graph of typed nodes - triggers (`manual` / `schedule` / `webhook` / `event`), **agent** nodes (an LLM turn scoped to the tools attached to it as `ai_tool` sub-nodes, auto-approving since unattended), deterministic **action** nodes (one tool, no LLM), **if** branches, and **set** transforms - joined by n8n-style `{ nodes, connections }` (see §3.8). A **React Flow** canvas builds and edits them (drag from a palette, wire tool sub-nodes into an agent's tool port, per-node config); **Generate** authors a whole graph from a plain-language prompt; **Run** executes it and overlays per-node status; and **schedule** triggers fire for real via a cron `tick` endpoint. Everything runs on the same harness as chat, with run history + a per-node transcript - the "trigger-agnostic by construction" claim, now with a real graph on top. And it produces **real outcomes**: agents `save_report` to a durable **Reports** section, `email_report` / `upload_to_drive` deliver those artifacts through the integration seam (mock-connected email + Google Drive), and every completed run drops a **notification** with a deep link to its results (the topbar bell).
...
- **Durable workflow runtime.** The graph engine, the node types, the React Flow canvas, AI graph-authoring, and **schedule** firing (a cron `tick` endpoint that runs due workflows and re-arms them) are all built. What's deferred is the **durable-execution runtime behind the `WorkflowRuntime` seam** - the in-process executor runs the graph today; a production deployment would hand firing, retries, and long waits to Inngest/Temporal while keeping the exact same node executor as the step body. `webhook`/`event` trigger *nodes* exist and validate; wiring their live entry points (an inbound webhook route, the domain event bus) is the remaining plumbing. This is the same queue seam as the >10k-row import path (same chunk functions, different trigger).
...
- **Live external sync** behind the mock provider seams - real Metrc/BioTrack, QuickBooks, and LeafLink adapters (the interfaces exist; only the live calls don't); deeper cultivation (plant lifecycle events, waste, mother plants) beyond the batch→plant→harvest core; and the **Billing** screen + notification-preferences settings (no backend yet). More import targets (purchase orders, assemblies).
