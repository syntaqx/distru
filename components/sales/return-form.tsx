...
import {
  createReturnAction,
  type ReturnLineForm,
} from "@/app/(app)/sales/returns/actions";
...
export function ReturnForm({
  ...
  const setLine = (idx: number, patch: Partial<ReturnLineForm>) =>
  ...
  function save() {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        ...
        setError(res.error ?? "Could not create return.");
      }
    });
  }
    ...
    <div className="mx-auto max-w-4xl">
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted">
            Record returned goods against a customer - a received return
            restocks its line items.
          </p>
        </div>
        <Link href="/sales/returns" className="btn btn-ghost">
          <X size={16} /> Cancel
        </Link>
      </div>
      ...
      <div className="space-y-4">
        <section className="card">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className={label}>Original order (optional)</label>
              <Combobox
      ...
      <div className="sticky bottom-0 mt-6 flex items-center justify-between gap-2 border-t bg-bg/80 py-3 backdrop-blur">
        ...
        <div className="flex gap-2">
          <Link href="/sales/returns" className="btn btn-outline">
            Cancel
          </Link>
          <button
